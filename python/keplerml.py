...
from datetime import datetime
from jupyter_core.application import raw_input
...
def fl_files_w_path(fl, fitsDir='./fitsFiles/', fl_as_array=False):
    ...
    if fl_as_array:
        ...
        if os.path.isfile(fl_c):
            ...
            # allows appending the fits directory later
            files = np.char.array(files)

        else:
            # char.array allows appending the fits directory later
            files = np.char.array(files)
...
def easy_feats(t, nf, err):
    ...
    # Q: Is there a way to do this and take into account drastically different
    # periodicity scales?
    ...
    if len(tflatness) == 0:
        ...
        tflatmean = np.mean(tflatness)  # F56
    ...
    if len(roundness) == 0:
        ...
        roundmean = np.mean(roundness)  # F57
    ...
    if len(troundness) == 0:
        ...
        troundmean = np.mean(troundness)  # F58
    ...
    return (
        meanmedrat, varss, coeffvar, stds, numoutliers, numnegoutliers,
        numposoutliers, numout1s, mad, meanpslope, meannslope, g_asymm,
        rough_g_asymm, diff_asymm, varabsslope, varslope, meanabsslope,
        absmeansecder, num_pspikes, num_nspikes, num_psdspikes, num_nsdspikes,
        stdratio, pstrend, num_pm, len_nmax, len_nmin, ptpslopes, oeratio,
...
def fancy_feats(t, nf, err, naivemax, maxdiff):
    ...
    longtermtrend = np.polyfit(t, nf, 1)[0]  # F1
    yoff = np.polyfit(t, nf, 1)[1]  # Not a feature, y-intercept of linear fit
    skews = stats.skew(nf)  # F3
    # this removes any linear trend of lc so you can look at just troughs
    ...
    return (
        longtermtrend, skews, kurt, maxslope, minslope, skewslope,
        num_zcross, mautocorrcoef, periodicity, periodicityr, naiveperiod,
        maxvars, maxvarsr, amp, normamp, mbp, mid20, mid35, mid50, mid65,
...
def feats(t, nf, err):
    ...
    returns:
        ndata (list, dtype varied) - the features in order as specified in
        the feature key (external to this code)
    """
    ...
    (
        longtermtrend, skews, kurt, maxslope, minslope, skewslope,
        num_zcross, mautocorrcoef, periodicity, periodicityr,
        naiveperiod, maxvars, maxvarsr, amp, normamp, mbp, mid20,
        mid35, mid50, mid65, mid80, percentamp, magratio, sautocorrcoef,
        autocorrcoef
        ) = fancy_feats(t, nf, err, naivemax, maxdiff)
    ...
    ndata = [
        ...
        absmeansecder, num_pspikes, num_nspikes, num_psdspikes, num_nsdspikes,
        stdratio, pstrend, num_zcross, num_pm, len_nmax, len_nmin,
        mautocorrcoef, ptpslopes, periodicity, periodicityr, naiveperiod,
        maxvars, maxvarsr, oeratio, amp, normamp, mbp, mid20, mid35, mid50,
...
def feature_calc(lc):
    ...
    try:
        ...
        fts = [
            "longtermtrend", "meanmedrat", "skews", "varss", "coeffvar",
            "stds", "numoutliers", "numnegoutliers", "numposoutliers",
            "numout1s", "kurt", "mad", "maxslope", "minslope", "meanpslope",
            "meannslope", "g_asymm", "rough_g_asymm", "diff_asymm",
            "skewslope", "varabsslope", "varslope", "meanabsslope",
            "absmeansecder", "num_pspikes", "num_nspikes", "num_psdspikes",
            "num_nsdspikes", "stdratio", "pstrend", "num_zcross", "num_pm",
            "len_nmax", "len_nmin", "mautocorrcoef", "ptpslopes",
            "periodicity", "periodicityr", "naiveperiod", "maxvars",
            "maxvarsr", "oeratio", "amp", "normamp", "mbp", "mid20", "mid35",
            "mid50", "mid65", "mid80", "percentamp", "magratio",
        ...
        kml_log = 'kml_log'
        os.system('echo %s ... TYPE ERROR >> %s' % (
            nfile.replace(fitsDir, ""), kml_log)
            )
        return
...
def join_data(full_features_array):
    ...
    fts = [
        ...
        "oeratio", "amp", "normamp", "mbp", "mid20", "mid35", "mid50",
        "mid65", "mid80", "percentamp", "magratio", "sautocorrcoef",
        "autocorrcoef", "flatmean", "tflatmean", "roundmean", "troundmean",
        "roundrat", "flatrat"
        ]
...
def features_from_filelist(fl,
    ...
    if verbose:
        ...
        print(f"Importing {len(files)} lightcurves...")
    ...
    for i in range(nchunks):
        ...
        p = Pool(useCpus)
        # Method saves to tmp_data.p file after processing each lightcurve as
        # a failsafe.
        ...
        if verbose:
            print(
                f"""
                {(i+1)*10000}/{nlcs} completed. Time for chunk: {datetime.now()
                -chunkStartTime}
                """
                )
...
if __name__ == "__main__":
    ...
    if sys.argv[3]:
        ...
        if of == "":
            print(
                """
                No output path specified, saving to output.csv in local folder.
                """
            )
            of = 'output.csv'
    start = datetime.now()
