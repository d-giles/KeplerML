"""
This code is developed to generate outlier scores objects in databases of arbitrary dimensionality.
Scores are based on the k-Nearest Neighbor (kNN) distance. This kinship is inversely proportional to outlier score, as outliers do not relate to other data.
"""
import numpy as np
...
def dist_scores(data,d2s=None,kmin=4,kmax=None):
    ...
    Args:
        data (Numpy array or Pandas dataframe) - The reference data to which distances will be calculated
        d2s (Numpy array or Pandas dataframe) - Data to be scored (if None, same as the reference data)
    ...
    if type(d2s) == type(None): 
        ...
        # every value of the dataframe to None, instead we check if it's NoneType
        d2s= data # will default to full data if d2s is not specified
    kmax = kmax or kmin
    assert kmax>= kmin, "Maximum value of k given is smaller than the minimum."
    ...
    # Idea: readjust scaling so that the extreme outliers don't dominate scores of the rest.
...
def kinship_scores(data,d2s=None,kmin=4,kmax=None,samp_size=-1,n_iter=1):
    ...
    Purpose:
        Calculate scores for each point within the dataset in an efficient manner. 
        The score is based on the distance to the k-Nearest Neighbor, 
        ...
        high density, high dimensional data is not particularly efficient or informative.
    """
    if samp_size>1:
        ...
        ref_data_sample = data # if samp_size isn't specified well, scoring is done relative to the whole dataset
